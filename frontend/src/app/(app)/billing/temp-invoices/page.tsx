...
import { Plus, Search, Ban, Printer, Trash2 } from 'lucide-react';
...
// Temp invoices — software-only marker (still prints as a normal invoice on
// the PDF). Kept on their own page so the main Invoices list is strictly
// TAX_INVOICE and doesn't mix temp with final.
export default function TempInvoicesPage() {
  const qc = useQueryClient();
  const [search, setSearch] = React.useState('');
  const { fy } = useFiscalYear();
  const q = useQuery<any[]>({
    queryKey: ['temp-invoices', { search, fy: fy.startYear }],
  ...
  const cancel = useMutation({
    ...
    onSuccess: () => { toast.success('Cancelled.'); qc.invalidateQueries({ queryKey: ['temp-invoices'] }); },
    onError: (e) => toast.error(getApiError(e).message),
  });
  const remove = useMutation({
    mutationFn: (id: number) => Api.billing.deleteInvoice(id),
    onSuccess: () => { toast.success('Deleted.'); qc.invalidateQueries({ queryKey: ['temp-invoices'] }); },
    onError: (e) => toast.error(getApiError(e).message),
  });
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Temp Invoices"
        description="Temporary invoices — software-only marker. The printed PDF matches the tax invoice layout."
        actions={
          <Link href="/billing/invoices/new?type=TEMP_INVOICE">
            <Button><Plus className="size-4" /> New Temp Invoice</Button>
          </Link>
        }
      />
            ...
            <div className="table-scroll">
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {(q.data ?? []).length === 0 && (
                    <tr><td colSpan={9} className="px-4 py-12 text-center text-muted-foreground">No temp invoices yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
